...
namespace DarkSoulsMemory.Internal
{
	...
	internal abstract class TrackerBase
	{
		...
		public ItemState[] GetItemStates(int baseId, int category)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				int count = ReadByte(Handle, address + countOffset);
				int mods = 0;
				int reinforcement = 0;
        ...
        protected byte ReadByte(IntPtr handle, IntPtr address)
        {
            int bytesRead = 0;
            byte[] bytes = new byte[1];

            Kernel32.ReadProcessMemory(handle, address, bytes, bytes.Length, ref bytesRead);

            return bytes[0];
        }

        protected int ReadInt32(IntPtr handle, IntPtr address)
        {
            int bytesRead = 0;
        ...
        protected byte[] ReadBytes(IntPtr handle, IntPtr address, int count)
        {
            int bytesRead = 0;
