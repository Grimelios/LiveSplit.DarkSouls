...
{
	internal class ItemTracker : TrackerBase
	{
		...
		{
			totalItems = ReadInt32(Handle, itemCount);
		...
		{
			int newTotal = ReadInt32(Handle, itemCount);
		...
		{
			return ReadInt32(Handle, address);
		}

		private int GetCategory(IntPtr address)
		{
			int value = ReadByte(Handle, address - 0x1);
